/* The following functions implement the desired basic functionality of the system */

-- FIXME replace function oeoe
-- creates a queue and returns the id of the newly created queue
CREATE OR REPLACE FUNCTION create_queue(p_name varchar(20))
RETURNS integer AS $$
DECLARE
...
-- send message, inserts message in the relation
-- in case receiver_id is NULL this means there is no specific receiver
CREATE OR REPLACE FUNCTION send_message(p_sender_id integer, p_receiver_id integer, p_queue_id integer,
...
--
--   -- why only checking the first parameter FIXME
--   IF p_requesting_user_id IS NULL THEN
--     RAISE EXCEPTION 'INVALID_REQUESTING_USER with NULL user_id';
--   END IF;
--
...
-- will actually receive the message.
...
CREATE OR REPLACE FUNCTION receive_message_from_sender(
  p_requesting_user_id INTEGER, /* id of the user issuing the receive message request */
...
BEGIN
  ...
  IF p_requesting_user_id = p_sender_id THEN
     RAISE EXCEPTION 'INVALID_SENDER with (sender_id)=(%)', p_sender_id;
  END IF ;
