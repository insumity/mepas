...
conf = \
    {"nameOfTheExperiment": "../NEW_NEW_increasing_clients",
...
def threadCode(values):
    ...
    # NOTICE!! if you are going to change this CHANGE THE CLIENT instantiation
    conf = \
        {"nameOfTheExperiment": "../NEW_NEW_increasing_clients", "placement": "us-west-2c", "databaseType": "m3.large",
         "clientInstances": (1, "m3.large"), "middlewareInstances": (1, "m3.large"), "databaseUsername": "ubuntu",
         "databasePassword": "mepas$1$2$3$", "databaseName": "mepas", "databasePortNumber": 5432,
         "middlewarePortNumber": 6789, "runningTimeInSeconds": 600, "threadPoolSize": 20, "connectionPoolSize": 20,
         "totalClients": 50, "totalQueues": 50, "messageSize": 20, "mappings": [(0, 0)], "clientsData": [(50, 1)],
         "username": "ubuntu", "variable": "totalClients", "values": values}
    ...
    for variable in conf["values"]:
        ...
        for mapping in conf["mappings"]:
            ...
            client = Client(conf["username"], clientIPs[mapping[0]][0], privateIPOfCorrespondingMiddleware,
                            str(conf["middlewarePortNumber"]), str(variable),
